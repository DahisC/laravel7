...
@section('content')
    ...
    @foreach ($productTypeList as $productType)
        <a href="{{ route('admin.product.index', ['type' => $productType->id]) }}">
        <span class="badge @if ($qs_type==$productType->type) badge-primary @else
                badge-secondary @endif">{{ $productType->type }}</span>
        </a>
    @endforeach
    <hr />
                ...
                <div class="d-flex justify-content-between align-items-end">
                    <h6 class="m-0 font-weight-bold text-primary">
                        ...
                        @foreach ($product->type ?? [] as $type)
                            <span class="badge badge-secondary">{{ $type->type }}</span>
                        @endforeach
                    </h6>
                </div>
            </a>
                    ...
                    <div class="d-flex justify-content-between flex-column flex-md-row">
                        ...
                        <div class="d-flex justify-content-end">
                            ...
                            <button class="btn btn-danger btn-icon-split btn-md"
                                onclick="document.querySelector('#delete-form{{ $product->id }}').submit();">
                                <span class="icon text-white-50">
