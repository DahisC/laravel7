  ...
  <form class="user"
    ...
    <div class="form-group">
      ...
      <div id="uploaded_images_wrapper" class="d-flex flex-wrap">
        @if (isset($product))
          @foreach ($product->images as $image)
            <img style="height: 200px; max-width: 100%;" class="d-block mx-auto my-3 rounded" src="{{ $image->url }}"
              data-id="{{ $image->id }}" />
          @endforeach
        @endif
      </div>
      ...
      <div class="d-flex">
        ...
        <input hidden id="input_images" name="images[]" type="file" accept="image/*" onchange="previewImages(this.files);"
          multiple />
      </div>
