...
@section('content')
  ...
  <hr />
  <form id class="user"
    action="{{ $action == 'Create' ? route('admin.news.store') : route('admin.news.update', ['news' => $news->id]) }}"
    method="POST" enctype="multipart/form-data">
    @csrf
    @method($action == "Create" ? "POST" : 'PUT')
    <div class="form-group">
      <label for="title">Title</label>
      <input id="title" type="text" class="form-control form-control-user" name="title"
        value="{{ $news->title ?? '世界が癒される国産ビートメイカーDJ OKAWARIとは？' }}" required autofocus placeholder="Title">
    </div>
    <div class="form-group">
      <label for="date">Date</label>
      <input id="date" type="date" class="form-control form-control-user" name="date"
        value="{{ $news->date ?? '2021-05-01' }}" required placeholder="Date">
    </div>
    <div class="form-group">
      ...
      <div class="d-flex">
        ...
        <button type="button"
          ...
          onclick="input_upload_img.click();">
    ...
    <div class="form-group">
      ...
      <input id="summernote_previewImages" name="summernote_previewImages[]" multiple type="file" hidden />
      <textarea id="summernote" class="form-control" rows="10"
        name="content">{{ $news->content ??
    '
    ...
    window.onload = () => {
      ...
      $('#summernote').summernote({
        callbacks: {
          ...
          //     } = await result.json();
          //     url.forEach(u => {
          //       $('#summernote').summernote('insertImage', u);
          //     })
          //   },
          onImageUpload: function(files) {
            const arr = [...summernote_previewImages.files, ...files];
            console.log(arr);
            // summernote_previewImages.files = files;
            [...files].forEach((file) => {
              $('#summernote').summernote('insertImage', URL.createObjectURL(file));
            })
          },
          onMediaDelete: function(test) {
            console.log(test);
          }
        }
      });
